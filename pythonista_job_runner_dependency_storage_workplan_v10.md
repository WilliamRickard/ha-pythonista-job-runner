...
- added package storage usage accounting across pip cache, wheelhouse, reusable virtual environments, and package report storage
- enforced `package_cache_max_mb` with best-effort least-recently-used pruning while protecting active reusable environments
- added startup prune support tied to `package_cache_prune_on_start`
- added manual cache prune and purge actions through the authenticated API and Advanced Web UI panel
- added optional hash-enforcement mode for `requirements.lock` files when **Require package hashes** is enabled
- added validations for suspicious public package paths, malformed local wheels, and oversized public wheel imports
- added support-bundle, audit, and job diagnostics coverage for prune and purge activity

Implementation notes:

- pruning is best-effort rather than a hard quota. The add-on now records current usage and removes least-recently-used private package content until it is back near the configured target, but it does not try to enforce filesystem-level quotas.
- active environment protection now covers all environments attached to running jobs, not just the most recently attached one. The executor registers active environment keys before launch and releases them when the job exits.
- manual purge is intentionally explicit. Cache purge clears private cache areas by default and can also remove reusable virtual environments and imported wheels when those flags are requested.
- hash enforcement currently applies to `requirements.lock` inputs and package profiles that resolve to lock files. Loose `requirements.txt` flows are unchanged.

Acceptance criteria status:

- cache does not grow without bound: implemented by usage accounting plus prune on startup and post-install/profile-build prune
- active environment is never pruned mid-run: implemented by active environment key registration and prune exclusion
- prune actions are visible in audit and diagnostics: implemented through audit events, package metadata, result summaries, support bundle data, and Web UI cache views
...
- added package integration entities for package cache size, reusable venv count, last prune status, and default profile
- added service actions for profile build, cache prune, and cache purge
- added a combined add-on API endpoint at `/packages/summary.json` for the Home Assistant integration surface
- exposed package summary, package cache, and package profile payloads through the custom integration diagnostics flow
- updated service descriptions and translations for the new package actions
- extended system health output with package cache and reusable environment details

Implementation notes:

- the Home Assistant integration now fetches a combined package summary payload on each coordinator refresh and falls back to stats-derived values if the dedicated endpoint is not available yet. That keeps the custom integration backward-tolerant during rolling upgrades.
- package actions were added as Home Assistant service actions instead of new button entities because they are operational admin tasks rather than frequently used primary controls. The package subsystem state itself is surfaced as diagnostic sensors.
- diagnostics now include both the add-on support bundle and the package-specific endpoints so a single Home Assistant diagnostics download carries the package subsystem state needed for troubleshooting.

Acceptance criteria status:

- package state is visible from Home Assistant proper, not only the add-on Web UI: implemented by new diagnostic sensors and extended system health
- service actions can trigger profile build and prune work: implemented by new integration service actions and client methods
- diagnostics download includes package subsystem state: implemented by new package diagnostics payloads
...
- updated `README.md`, `pythonista_job_runner/README.md`, and `pythonista_job_runner/DOCS.md` for package-mode guidance and migration notes
- added two new package screenshot placeholders plus updated screenshot filename contract docs
- added a dedicated `examples/packages/` track with three package-focused examples:
  - `11_cached_per_job_requirements`
  - `12_offline_wheelhouse_install`
  - `13_named_package_profile_run`
- updated example docs and runner docs to explain package modes, package result artefacts, and required public-config setup
- added deterministic `expected_result/` artefacts and built `job.zip` files for the three package examples
- added validator and test coverage for the new package examples

Implementation notes:

- the package examples were kept separate from the earlier core examples because they need package subsystem settings or public add-on config files prepared first
- `11_cached_per_job_requirements` deliberately uses a vendored wheel so it stays offline-safe while still demonstrating reuse on the second run
- `12_offline_wheelhouse_install` and `13_named_package_profile_run` both ship `public_config/` assets that should be copied into the add-on public config area before running the job
- the new screenshot files are placeholders, not production captures. They satisfy the docs contract until real package UI screenshots are captured

Acceptance criteria status:

- Example 5 is replaced or expanded to prove the new model clearly: implemented by keeping Example 5 as the bridge example and adding three dedicated package examples
- docs explain when to use per-job mode versus profile mode: implemented in `DOCS.md` and both README files
- examples include success criteria and troubleshooting: implemented in the new package example READMEs
...
Status: implemented in this repo update, with manual native-host sign-off and backup-restore execution still pending outside the repository environment.

Deliverables completed in this repo update:

- fixed the repository architecture truthfulness wording so the packaging guardrails and user-facing docs agree on the exact CI validation scope
- added `docs/RELEASE_READINESS.md` as the release gate for regression, upgrade, backup-restore, and architecture sign-off
- updated root and add-on README files plus `DOCS.md` to link the release-readiness checklist and explain what still needs real-host validation
- updated `CHANGELOG.md` and release-channel docs so package subsystem work and sign-off expectations are visible in one place
- ran the repository guardrail, docs, package example, and compile validation suite for this coding phase
...
- no regression to current basic job execution: covered by the repository guardrail, package, and compile checks run in this phase; real-host example reruns remain the manual release gate
- package features degrade cleanly when disabled: covered by repository tests and docs, with final host validation still required
- user-visible docs and Web UI are consistent: updated in this repo phase
...
### 2026-03-10 Phase 6 update

Implemented package storage safety rails. The repo now accounts for private package storage usage, prunes least-recently-used cache content to stay near `package_cache_max_mb`, protects active environments attached to running jobs, and exposes manual cache prune and purge actions through both the add-on API and the Advanced Web UI.

New files introduced in Phase 6:

- `pythonista_job_runner/app/runner/package_prune.py`
- `pythonista_job_runner/app/runner/package_hashes.py`
- `pythonista_job_runner/tests/test_package_prune.py`
- `pythonista_job_runner_dependency_storage_workplan_v7.md` in the repo root

Files materially changed in Phase 6:
...
Known carry-forward items into Phase 7:

- package cache state is still primarily visible in the add-on Web UI and API rather than Home Assistant entities
- profile build, prune, and purge actions are not yet exposed as Home Assistant services
- custom integration diagnostics do not yet include the package subsystem state directly

### 2026-03-10 Phase 8 update

Implemented the documentation, examples, and migration phase. The repo now has a dedicated package examples track, package-mode migration guidance, updated screenshot placeholders, and three concrete package-focused examples that cover repeated per-job reuse, offline wheelhouse installs, and named package profile attachment.

New files introduced in Phase 8:

- `pythonista_job_runner/examples/packages/README.md`
- `pythonista_job_runner/examples/packages/11_cached_per_job_requirements/...`
- `pythonista_job_runner/examples/packages/12_offline_wheelhouse_install/...`
- `pythonista_job_runner/examples/packages/13_named_package_profile_run/...`
- `pythonista_job_runner/tests/test_examples_packages.py`
- `docs/screenshots/04_packages_overview.png`
- `docs/screenshots/05_package_profiles.png`
- `pythonista_job_runner_dependency_storage_workplan_v9.md` in the repo root

Files materially changed in Phase 8:

- `README.md`
- `pythonista_job_runner/README.md`
- `pythonista_job_runner/DOCS.md`
- `docs/screenshots/README.md`
- `pythonista_job_runner/examples/README.md`
- `pythonista_job_runner/examples/core/README.md`
- `pythonista_job_runner/examples/manifest.json`
- `pythonista_job_runner/examples/tools/validate_examples.py`
- `pythonista_job_runner/examples/tools/manifest_schema.md`
- `pythonista_job_runner/examples/tools/README.md`
- `pythonista_job_runner/examples/tools/pythonista_example_runner_README.md`

Known carry-forward items into Phase 99:

- package examples still need real user validation runs and checked-in validation evidence
- placeholder package screenshots should be replaced with real captures once the package UI has been exercised on-device
- Example 05 success-path user validation is still pending and should be reviewed alongside the new package examples


### 2026-03-10 Phase 99 update

Implemented the final repository hardening pass for the dependency-storage workstream. This phase added an explicit release-readiness checklist, updated release-facing docs and changelog entries, aligned the architecture truthfulness wording with the packaging guardrails, and treated the repository guardrail plus package example checks as the final in-repo validation gate for this coding step.

New file introduced in Phase 99:

- `docs/RELEASE_READINESS.md`
- `pythonista_job_runner_dependency_storage_workplan_v10.md` in the repo root

Files materially changed in Phase 99:

- `README.md`
- `pythonista_job_runner/README.md`
- `pythonista_job_runner/DOCS.md`
- `pythonista_job_runner/CHANGELOG.md`
- `docs/RELEASE_CHANNELS.md`

Carry-forward manual sign-off items after Phase 99:

- rerun the package examples on a real Home Assistant host and check in validation evidence
- verify backup and restore of `/config/package_profiles/` and `/config/wheel_uploads/` on a real host
- complete native-host smoke testing for `aarch64` and `armv7` before stable release sign-off
