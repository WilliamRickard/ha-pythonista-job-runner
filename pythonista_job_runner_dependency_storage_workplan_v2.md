...
- add wheelhouse manager under `/data/pythonista_job_runner/wheelhouse/`
- support `pip download` and `pip wheel` preparation into wheelhouse
- support `--find-links` and optional `--no-index` install flow
- add import flow from `/config/wheel_uploads/` into validated internal wheelhouse
- add package diagnostics for wheelhouse hits and misses
- add a simple wheelhouse status view in Web UI

Acceptance criteria:

- a job can install from local wheelhouse without remote index access
- imported wheels from `/config` are validated and copied into `/data`
- logs make clear whether install used remote index, local wheelhouse, or both

### Phase 4. Reusable virtual environments

Deliverables:

- add keyed venv creation and reuse using `venv`
- build environment keys from Python version, architecture, dependency definition hash, and policy flags
- attach venv interpreter or site-packages cleanly to job runs
- add environment index and last-used tracking
- add `venv_max_count` cap and pruning logic

Acceptance criteria:

- second run with identical resolved requirements reuses the same venv
- runs with changed requirements build a new venv instead of mutating the old one
- venv reuse works without polluting unrelated jobs

### Phase 5. Package profiles

Deliverables:

- add `/config/package_profiles/` layout and profile manifest support
- add profile build and rebuild commands
- support `dependency_mode: profile`
- add default profile selection in config
- add profile table and actions in Web UI
- add exported effective lock file and diagnostics bundle

Acceptance criteria:

- user can define a profile once and reuse it across multiple jobs
- profile build is visible in Web UI and API
- job run clearly shows which profile was attached
